...
class ERRORA(object):
    ...
    def __init__(self, config):
        ...
        self.train_loader = get_loader(config, 'error')
    ...
    def build_model(self):
        ...
        if self.encoder_multi:
            ...
            self.E = E_o(start_channel=self.e_start_ch,
                         target_channel=self.encoder_last_ch, block=self.block,
                         LR=self.LR, spec=self.g_spec).to(self.gpu)
            self.E.eval()
    ...
    def run(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(data_loader)):
                ...
                outfit_id, t_idx, _, source_images = data
                ...
                #print(f'latent_code : {latent_code}')

                if self.use_emd:
                    latent_code = self.Emd(latent_code)
                    #print(f'emd latent_code : {latent_code}')
                ...
                sample_path = osp.join(self.sample_dir, f'{i}_error_analysis.jpg')# .zfill(filling)
